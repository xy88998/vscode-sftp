...
let cachedVersion: string | undefined; // 缓存的当前版本
let lastUsedVersion: string | undefined; // 最后一次use的版本
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const terminalOpenDisposable = vscode.window.onDidOpenTerminal(async (terminal) => {
        const config = vscode.workspace.getConfiguration('vscode-nvm');
        const autoSwitchEnabled = config.get('autoSwitch', false);

        if (autoSwitchEnabled) {
            // 使用 setTimeout 以确保终端已准备好接收命令
            setTimeout(async () => {
...
async function getCurrentNodeVersionForced(): Promise<string> {
    try {
